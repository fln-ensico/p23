...
pretty = (IHaskell.Display.html . htm . htable) 

--- DOT, GraphViz API

g2lg = --- converts graph to string-labelled graph (empty labels)
   map (split (show><show) (const ""))
...
picd = IHaskell.Display.html . htm . planoxy 0.9

picd' = IHaskell.Display.html . htm . planoxy 0  -- gray intensity = 0

picd'' = IHaskell.Display.html . htm . wrap . concat
...
histd_old = shw_h . sort . hist

shw_h x = let y=zip [ 15 * i + 3 | i <- [0..]] x
              n = length x
              s :: Int -> Float
              s n = 500 *  (fdiv n m) -- scale factor
              m = maximum . map snd $ x
              h = htm . svg' m n . title "histogram" . (>>= bar s) $ y
          in IHaskell.Display.html h
